/**
 * Portal Cost Simulator — Calculate employer costs and social insurance contributions
 * Supports both forward (Gross → Net) and reverse (Net → Gross) calculation modes.
 * Uses portal tRPC (portalTrpc.toolkit.*).
 */
...
export default function CostSimulator() {
  ...
  const forwardMutation = portalTrpc.toolkit.calculateCost.useMutation({
    onSuccess: (data) => setResult({ ...data, mode: "forward" }),
    onError: (err) => toast.error(err.message || "Calculation failed"),
  });

  const reverseMutation = portalTrpc.toolkit.calculateCostReverse.useMutation({
