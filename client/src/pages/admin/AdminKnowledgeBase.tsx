/**
 * Admin Knowledge Base — Browse published articles from the admin side.
 * Mirrors the Portal Knowledge Base UI but uses admin Layout and admin tRPC context.
 * No customer-specific filtering; shows all published articles.
 */
...
export default function AdminKnowledgeBase() {
  ...
  // Sort available countries alphabetically
  ...
  if (selectedItem) {
    ...
    return (
      <Layout title={`${t("knowledge_base.title")} — ${selectedItem.title}`}>
  ...
  // ─── List View ───
  return (
    <Layout title={t("knowledge_base.title")}>
                ...
                {topicList.map((topic) => {
                  ...
                  return (
                    <div key={topic} className="flex items-center gap-2">
                      ...
                      />
                      <Label htmlFor={`admin-kb-${topic}`} className="cursor-pointer">
                        {t(key)}
                        {topicCount > 0 && (
            ...
            {sortedCountryCodes.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {sortedCountryCodes.map((code, idx) => {
                    const isSelected = selectedCountries.includes(code);
                    const articleCount = availableCountries[code] ?? 0;
                    // Show first 20 countries by default, rest hidden unless showAllCountries
                    if (!showAllCountries && !isSelected && idx >= 20) return null;
                    return (
                      <Button
                        key={code}
