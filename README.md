...
3. 启动本地 PostgreSQL 数据库：
   ```bash
   docker run -d --name gea-pg -e POSTGRES_PASSWORD=postgres -p 5432:5432 postgres:16
   ```
4. 安装依赖：`pnpm install`
5. 执行数据库迁移：`pnpm run db:push`
6. 配置本地 Hosts 文件（以支持子域名路由）：
   在 `/etc/hosts` 中添加：`127.0.0.1 admin.geahr.com app.geahr.com worker.geahr.com`
7. 启动开发服务器：`pnpm run dev`
   - Admin 端: `http://admin.geahr.com:3000`
   - Portal 端: `http://app.geahr.com:3000`
   - Worker 端: `http://worker.geahr.com:3000`
...
- **前端**: React 19, Wouter, Tailwind CSS, shadcn/ui, React Query
