...
薪资计算（Payroll）是 EOR 业务的心脏。GEA 的薪资计算遵循一个铁律：**“本月发底薪，上月发异动（三大项）”**。

为了实现这个铁律，系统设计了一条严格的时间线。作为 Ops 和 Finance，你们必须在这个时间线内完成接力，任何提前或延后的操作，都会导致员工少拿钱或公司合规违约。
...
- **如果在 5 号之后发现漏了数据怎么办？** 如果客户在 6 号才想起来补发一笔 3 月的奖金，你可以利用 Admin 特权帮他补建。建完后，**必须让客户在 Portal 点击 Approve**，然后你再 Approve。最后，在 4 月的 `draft` 薪资单里点击 **Auto-fill**，这笔迟到的奖金就会被重新拉取进来。
...
- 对于 18 号入职的**李四**：因为超过了 15 号这个月中分界线，系统激活他后，**不会**把他放进 4 月薪资单。相反，系统会自动生成一笔名为 "Sign-on Bonus" 的 Adjustment，这笔钱等于他 4 月该拿的工资，但生效月份是 5 月。李四将在 5 月的工资条里，一次性拿到 4 月和 5 月的钱。
...
- 同时，系统会触发生成一笔押金账单（Deposit Invoice）给客户。
...
**系统接管与硬性规则：**
- **硬规则 1：离职员工必须发工资。** 赵六处于 `offboarding` 状态期间，系统依然会把他拉进 5 月的薪资单。
- **硬规则 2：离职员工必须继续攒年假。** 每月 1 号的年假累计任务（Job 7），不仅会给 `active` 员工发年假，也会给 `offboarding` 员工发年假。
- **对称 Pro-rata：** 在 5 月的薪资单中，系统发现赵六的 End Date 是 5 月 20 号，会自动计算 1-20 号的 Pro-rata 工资，不会多发一分钱。
- **自动终止：** 5 月 20 号凌晨 00:02（Job 8: Auto-Termination），系统发现赵六的 End Date 到了，自动将他从 `offboarding` 变为 `terminated`，并触发押金退还流程。
...
- [ ] **退款账单的抵扣**：员工离职后系统会生成 Deposit Refund Invoice。在开下个月账单时，记得用这笔 Refund 去抵扣（Applied）客户的新账单。
...
3. **Job 3 (Auto-Create Payroll Runs)**: 每月 5 号 00:01。建薪资单，抓取 `locked` 数据，算钱。生成承包商发票。
