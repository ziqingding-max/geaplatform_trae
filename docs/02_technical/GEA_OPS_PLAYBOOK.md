...
- **自动终止：** 5 月 20 号凌晨 00:02（Job 8: Auto-Termination），系统发现赵六的 End Date 到了，自动将他从 `offboarding` 变为 `terminated`，并触发押金退还账单（Deposit Refund Invoice）。请注意，退还账单生成后，金额会退回到客户的**冻结钱包（Frozen Wallet）**，后续需手动解冻并转移至可用钱包。
