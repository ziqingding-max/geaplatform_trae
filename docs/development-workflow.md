...
| API | tRPC 11 + SuperJSON | `server/routers/`, `server/portal/routers/` |
| 后端 | Node.js 22 + Express 4 | `server/_core/` |
| ORM | Drizzle ORM | `drizzle/schema.ts`, `server/db.ts` |
| 数据库 | MySQL 8 / TiDB Serverless | `drizzle/migrations/` |
...
**第三步：tRPC Procedure。** 在 `server/routers/` 中创建或扩展路由文件，选择 `publicProcedure`（无需认证）或 `protectedProcedure`（需要登录）。使用 Zod 定义输入校验，通过 `ctx.user.role` 进行权限检查。如果是客户门户功能，在 `server/portal/routers/` 中创建，使用 `protectedPortalProcedure`（自动注入 `ctx.customerId`）。

**第四步：前端 UI。** 在 `client/src/pages/` 中创建页面组件，通过 `trpc.*.useQuery / useMutation` 调用后端。使用 Shadcn/UI 组件保持一致性，在 `client/src/lib/i18n.ts` 中添加中英文翻译。在 `client/src/App.tsx` 中注册路由。
...
当路由文件超过 150 行时，拆分到 `server/routers/<feature>.ts`。当前系统已拆分为 18 个管理后台路由和 9 个门户路由。
...
| L3 | 服务健康检查 | `webdev_check_status` | 每次重启后 |
...
每次 `webdev_save_checkpoint` 生成唯一的 `version_id`（8 位哈希）。版本语义遵循 SemVer 规范：Major 版本对应重大功能模块上线，Minor 版本对应新功能或子模块完成，Patch 版本对应 Bug 修复和 UI 微调。

回滚策略按优先级排列：首先尝试代码修复（保持前进），其次使用 `webdev_rollback_checkpoint` 回到稳定版本，最后通过 SQL 脚本修复数据层面问题。

---

## 6. 发布检查清单

每次准备交付 checkpoint 前，需完成以下检查：

| 检查项 | 命令 / 方法 | 通过标准 |
|:---|:---|:---|
| 测试通过 | `pnpm test` | 全部通过 |
| 类型检查 | `npx tsc --noEmit` | 零错误 |
| 服务健康 | `webdev_check_status` | 服务正常 |
| 浏览器控制台 | 开发者工具 | 无错误 |
...
所有生产环境变更必须遵循 hotfix-governance 规范，包括代码修复、配置变更和数据操作。变更前需评估影响范围，变更后需验证功能正常，并在审计日志中记录操作。
